...
subscription = node.create_subscription(Image,"/count_penny_image", on_image, 10) # Creating the Subscribe node
...
def get_frame():
    global frame
    if frame is None:  # If no frame has been received yet
        return b''  # Return empty bytes
    
    # Try to get new frame with timeout
    if rclpy.ok():
        rclpy.spin_once(node, timeout_sec=1.0)
        if event.wait(timeout=1.0):  # Wait for new frame with 1 second timeout
            event.clear()
            return frame
        else:  # If timeout occurs, return last frame
            return frame
    return frame
